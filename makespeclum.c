...
#define SPEC_LUM 0.38

unsigned short spectrum[IMAGE_WIDTH][3];
...
#if USE_TRUE_CONE_SENSITIVITIES
      purelight_cone (exp2((((double)j)/(IMAGE_WIDTH-1))*-1.)*800.,
		      &l, &m, &s);
      cone_to_xyz (l, m, s, &x, &y, &z);
#else
      purelight_xyz (exp2((((double)j)/(IMAGE_WIDTH-1))*-1.)*800.,
		     &x, &y, &z);
#endif
      char out_of_lum;
      x *= SPEC_LUM;  y *= SPEC_LUM;  z *= SPEC_LUM;
      xyz_to_srgb16 (x, y, z, &r16, &g16, &b16, NULL, 0, 1., &out_of_lum);
      spectrum[j][0] = r16;
      spectrum[j][1] = g16;
      spectrum[j][2] = b16;
    }
...
make_image (void)
{
  ...
  for ( i=0 ; i<IMAGE_HEIGHT ; i++ )
    for ( j=0 ; j<IMAGE_WIDTH ; j++ )
      for ( k=0 ; k<3 ; k++ )
	{
	  image[i][j][k][0] = (spectrum[j][k]>>8);
	  image[i][j][k][1] = (spectrum[j][k]&0xff);
	}
}
